...
static void realloc_row(mr_sparse_row *row) {
    if (row->cap == 0) {
        ++row->cap;
        row->cols = xmalloc(sizeof(mr_int));
        row->values = xmalloc(sizeof(mr_float64));

        return;
    }

    row->cap *= 2;
    row->cols = xrealloc(row->cols, row->cap * sizeof(mr_int));
    row->values = xrealloc(row->values, row->cap * sizeof(mr_float64));
}

static void row_insert(mr_sparse_row *row, mr_int col, mr_float64 val, mr_index pos) {
    if (row->len == row->cap) {
        realloc_row(row);
    }
...
mr_sparse_matrix_builder *mr_sparse_matrix_builder_create(mr_int dim) {
    ...
    builder->rows = xmalloc((builder->dim + 1) * sizeof(mr_int));
    builder->rows[0] = 0;
...
void mr_sparse_matrix_builder_add_row(mr_sparse_matrix_builder *builder, mr_sparse_row *row) {
    ...
    mr_int old_nb_cols = builder->nb_cols;
    builder->nb_cols += row->len;
    builder->rows[++builder->nb_rows] = builder->nb_cols;

    builder->values = xrealloc(builder->values, builder->nb_cols * sizeof(mr_float64));
    memcpy(builder->values + old_nb_cols, row->values, row->len * sizeof(mr_float64));

    builder->cols = xrealloc(builder->cols, builder->nb_cols * sizeof(mr_int));
    memcpy(builder->cols + old_nb_cols, row->cols, row->len * sizeof(mr_int));
}

mr_sparse_matrix *mr_sparse_matrix_build(mr_sparse_matrix_builder *builder) {
    mr_sparse_matrix *mat = xmalloc(sizeof(mr_sparse_matrix));

    mat->dim = builder->dim;

    mat->values = builder->values;
